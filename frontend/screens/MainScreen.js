...
const BarTitle = ({ title }) => {
  ...
  return (
    <Animated.View style={tw`w-full pb-8`} entering={FadeInUp.duration(500)} key={navigation?.getState()?.index}>
      <Text style={tw`absolute left-3 font-bold text-3xl`}>{title}</Text>
    </Animated.View>
  )
}
...
const CustomDrawerContent = ({ userData, ...props }) => {
  return (
    <DrawerContentScrollView {...props}>
      <View style={tw`items-center flex-row py-5 border-gray-300`}>
        <Image 
          source={require('../assets/images/Logo.png')}
          style={tw`w-20 h-20 rounded-full mb-3`}
        />
        <View>
          <Text style={tw`text-lg font-bold`}>{userData.username}</Text>
          <Text style={tw`text-gray-500`}>{userData.email}</Text>
        </View>
      </View>

      <DrawerItemList {...props} />
      <View style={tw`mt-5 h-10 w-25 rounded-xl`}>
        <TouchableOpacity onPress={() => handleLogout(navigation)} style={tw`flex-row justify-center items-center`}>
          <Svg width="46" height="31" viewBox="0 0 46 31" fill="none" xmlns="http://www.w3.org/2000/svg">
            <Path d="M23 19.3751L28.75 15.5001M28.75 15.5001L23 11.6251M28.75 15.5001H7.66663M17.25 9.36284V9.30033C17.25 7.85353 17.25 7.12959 17.6678 6.57699C18.0353 6.0909 18.6213 5.69599 19.3426 5.44831C20.1626 5.16675 21.2368 5.16675 23.3837 5.16675H32.2003C34.3472 5.16675 35.4191 5.16675 36.2391 5.44831C36.9604 5.69599 37.5484 6.0909 37.9159 6.57699C38.3333 7.12905 38.3333 7.85212 38.3333 9.29609V21.7047C38.3333 23.1487 38.3333 23.8707 37.9159 24.4228C37.5484 24.9088 36.9604 25.3045 36.2391 25.5521C35.4199 25.8334 34.3486 25.8334 32.2059 25.8334H23.3774C21.2347 25.8334 20.1618 25.8334 19.3426 25.5521C18.6213 25.3045 18.0353 24.9085 17.6678 24.4224C17.25 23.8698 17.25 23.1469 17.25 21.7001V21.6355" stroke="#007AFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </Svg>
          <Text style={tw`text-white text-[#007AFF]`}>Logout</Text>
        </TouchableOpacity>
      </View>
...
const CustomHamburger = ({ navigation }) => {
  return (
    <Animated.View key={navigation.getState().index} entering={FadeInUp.duration(500)}>
      <Pressable onPress={() => navigation.toggleDrawer()} style={{ padding: 16 }}>
        <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
          <Path d="M19.5 19.7656H3.5" stroke={"#000"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
          <Path d="M19.5 5.76562H3.5" stroke={"#000"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
          <Path d="M19.5 12.7656H3.5" stroke={"#000"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
        </Svg>
      </Pressable>
    </Animated.View>
  )
...
const MainScreen = () => {
  ...
  const fetchUser = async () => {
    ...
    const user = Object.fromEntries(storedData)
    setUserData({username: user.username, email: user.email, id: user.id})
  }
  ...
  return (
    <Drawer.Navigator 
      ...
      screenOptions={({navigation}) => ({
        headerLeft: () => <CustomHamburger navigation={navigation}/>,
        headerStyle: tw`bg-[#E3E3E3] shadow-none border-0 h-25`,
        headerLeftContainerStyle: tw`pl-3`,
        drawerActiveBackgroundColor: "#007AFF",
        drawerActiveTintColor: "#FFFFFF",
        drawerInactiveTintColor: "#000000",
        drawerLabelStyle: tw`text-lg pb-1.5`,
        drawerItemStyle: tw`rounded-xl`,
        headerTitle: () => (
          <Animated.View key={navigation.getState().index} entering={FadeInUp.duration(500)}>
            <Image source={require('../assets/images/Logo.png')} style={tw`h-15 w-15`}/>
          </Animated.View>
        ),
      })
    }>
      <Drawer.Screen 
        ...
        options={{
        headerRight: () => <BarTitle title="Eira"/>, 
        drawerIcon: ({ focused }) => (
          <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <Path d="M3 20V4C3 3.44772 3.44772 3 4 3H20C20.5523 3 21 3.44772 21 4V16C21 16.5523 20.5523 17 20 17H6L3 20Z" 
              stroke={focused ? "#FFFFFF" : "#000000"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            />
          </Svg>
      )}}/>
      <Drawer.Screen 
